...
function drawCenterCrop(source) {
  const canvas = els.canvas;
  const ctx = canvas.getContext("2d", { willReadFrequently: true });
  const sourceWidth = source.videoWidth || source.naturalWidth || source.width;
  const sourceHeight = source.videoHeight || source.naturalHeight || source.height;
  if (!sourceWidth || !sourceHeight) throw new Error("Ukuran gambar belum tersedia.");

  const cropSide = Math.min(sourceWidth, sourceHeight) * config.cropRatio;
  const sx = (sourceWidth - cropSide) / 2;
  const sy = (sourceHeight - cropSide) / 2;
  canvas.width = config.inputSize;
  canvas.height = config.inputSize;
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ctx.drawImage(source, sx, sy, cropSide, cropSide, 0, 0, canvas.width, canvas.height);
}
...
async function classifySource(source, { resetSmoothing = false, sourceType = "camera" } = {}) {
  ...
  updateControlState();
  try {
    drawCenterCrop(source);
    const started = performance.now();
    const result = classifier.classifyForVideo(els.canvas, started);
    const latencyMs = performance.now() - started;
    const categories = result.classifications?.[0]?.categories || [];
    if (!categories.length) throw new Error("Model tidak mengembalikan hasil klasifikasi.");

    const vector = categoriesToVector(categories);
    const smoothed = smoothVector(vector, resetSmoothing);
    const index = bestIndex(smoothed);
    const label = config.labels[index];
    const confidence = smoothed[index];
    const accepted = confidence >= config.confidenceThreshold;

    renderResult({ label, confidence, accepted, latencyMs, probabilities: smoothed });
    appendLog({ label, confidence, accepted, latencyMs, sourceType });
  } catch (error) {
    console.error(error);
...
function renderResult({ label, confidence, accepted, latencyMs, probabilities }) {
  els.resultCard.classList.remove("empty");
  els.resultConfidence.textContent = `${(confidence * 100).toFixed(1)}%`;
  els.latency.textContent = `${latencyMs.toFixed(0)} ms`;
  els.delegate.textContent = delegateName;

  if (!accepted) {
    els.resultName.textContent = "Motif belum dikenali";
    els.resultMessage.textContent = `Confidence ${(confidence * 100).toFixed(1)}% berada di bawah ambang ${(config.confidenceThreshold * 100).toFixed(0)}%. Dekatkan kamera dan penuhi kotak dengan satu motif.`;
    els.educationPanel.classList.add("hidden");
  } else {
    const metadata = metadataMap.get(label) || {};
    els.resultName.textContent = displayName(label);
    els.resultMessage.textContent = "Motif dikenali. Baca informasi budaya yang telah divalidasi di bawah ini.";
    els.origin.textContent = metadata.origin || "Belum diisi";
    els.meaning.textContent = metadata.meaning || "Belum diisi";
    els.history.textContent = metadata.history || "Belum diisi";
    els.usage.textContent = metadata.usage_notes || "Belum diisi";
    els.source.textContent = metadata.source_reference || "Belum diisi";
...
function appendLog({ label, confidence, accepted, latencyMs, sourceType }) {
  scanLog.push({
    sample_id: `SCAN-${Date.now()}`,
    timestamp: new Date().toISOString(),
    true_label: els.trueLabel.value,
    predicted_label: accepted ? label : "unknown",
    top_label_before_threshold: label,
    ...
    angle_deg: els.angle.value,
    distance_cm: els.distance.value,
    medium: els.medium.value,
  });
  localStorage.setItem("batiklens_scan_log", JSON.stringify(scanLog));
  updateLogCount();
}
...
function downloadLog() {
  ...
  const columns = Object.keys(scanLog[0]);
